...
// read json file
/**@type {Array} */
let canteens_json = JSON.parse(readFileSync(join(__dirname, '..', 'canteens.json'), {encoding: 'utf-8'}))
...
if(existing_canteen){
    if(new_canteen.name && existing_canteen.name != new_canteen.name){
        ...
        writeFileSync(join(__dirname, '..', 'canteens.json'), JSON.stringify(canteens_json, undefined, 4), {encoding: 'utf-8'})
    } else{
